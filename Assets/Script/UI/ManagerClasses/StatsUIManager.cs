...
public class StatsUIManager : MonoBehaviour {
    ...
    public void UpdateUI() {
        ...
        string headerText = $"{charData.characterName} ({charData.characterClass})\nLevel {charData.currLevel}";
        header.GetComponent<TextMeshProUGUI>().text = headerText;

        UpdateField("Max Health", charData.baseMaxHealth, charData.maxHealth);
        UpdateField("Attack", charData.baseAttack, charData.attack);
        UpdateField("Magic Attack", charData.baseMagicAttack, charData.magicAttack);
        UpdateField("Defence", charData.baseDefence, charData.defence);
        UpdateField("Magic Defence", charData.baseMagicDefence, charData.magicDefence);
        UpdateField("Agility", charData.baseAgility, charData.agility);
    ...
    private void AddChoice(Equipment e, int i) {
        ...
        if (e != null) {
            ...
            var statBuffs = new List<(string, int)> {
                ("hp", e.healthBuff),
                ("atk", e.attackBuff),
                ("matk", e.magicAttackBuff),
                ("def", e.defenceBuff),
                ("mdef", e.magicDefenceBuff),
                ("agi", e.agilityBuff),
                ("luck", e.luckBuff)
            };
